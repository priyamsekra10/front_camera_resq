...
def run_object_detection_and_depth_estimation(video_path='vid2.mp4', depth_threshold=50):
    # Initialize YOLOv7 model with GPU support for object detection

    ######################################################################################################################################
    yolov7_model_path = "yolov7_384x640.onnx"
    yolov7_detector = YOLOv7(yolov7_model_path, conf_thres=0.5, iou_thres=0.5)

    # Constants for depth estimation using MiDaS
    ...
    while cap.isOpened():
        ...
        # Get the polygon vertices after the lane finding pipeline
        # poly_vertices = get_vertices(frame_with_lanes)
        ...
        # cv2.polylines(roi_mask, poly_vertices, isClosed=True, color=[0, 0, 255], thickness=10)
        cv2.fillPoly(roi_mask, [poly_vertices], (255, 255, 255))

        # Apply the mask to the frame
        frame_roi = cv2.bitwise_and(frame, roi_mask)

        # Object detection using YOLOv7 within the ROI
        boxes, scores, class_ids = yolov7_detector(frame_roi)

        # boxes, scores, class_ids = yolov7_detector(frame)

        
        # Filter out only cars (class ID 2) and trucks (class ID 7)
        ...
        # Draw bounding boxes on the original frame using draw_detections
        detection_img = draw_detections(frame_with_lanes, car_truck_boxes, car_truck_scores, class_ids=class_ids)

        # frame = lane_finding_pipeline(frame)

        # Resize and normalize the input frame for MiDaS
        input_frame1 = cv2.resize(frame, (midas_input_width, midas_input_height))

        input_frame = cv2.resize(frame, (midas_input_width, midas_input_height))
        input_frame = cv2.cvtColor(input_frame, cv2.COLOR_BGR2RGB)
        input_frame = input_frame.astype(np.float32) / 255.0

        # Reshape the input frame for MiDaS ONNX model
        ...
        # Draw bounding boxes on the depth estimation frame using draw_depth_boxes
        depth_with_boxes = draw_detections_midas(resized_depth, car_truck_boxes)

        

        # Check for tailgating and display an alert
        ...
        # Resize the original frame to match the height of the resized depth map
        resized_detection_img = cv2.resize(detection_img, (detection_img.shape[1], resized_depth.shape[0]))

        # Resize detection_img to match the height of depth_with_boxes
...
# Run the function
run_object_detection_and_depth_estimation()
